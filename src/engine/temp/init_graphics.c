...
/*   Updated: 2026/05/03 13:44:46 by yel-moha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_cell(t_game *game, int row, int col, char cell)
{
	...
	/* flip horizontally: convert column to mirrored column index */
	fcol = game->scene->map.width - 1 - col;
	pixel_x = PADDING + game->mini_map->off_width
		+ (fcol * game->mini_map->tile);
...
static void	draw_vlines(t_game *game)
{
	...
	{
		/* compute mirrored column for vertical grid line */
		fcol = game->scene->map.width - 1 - (col + 1);
